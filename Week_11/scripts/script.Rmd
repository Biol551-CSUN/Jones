...
"Mid"=" Mid",
"High"=" High")) %>% 
  mutate(Tide_Height2=factor(Tide_Height, levels=c("Low", "Mid", "High"))) %>% #order the rows of my data by low, then mid, then high tide for the x axis on the plot
  select(Stars, Crabs, Whelks, Site, Transect, Tide_Height2, Bare_Rock, Algae, Small_Barnacles, Large_Barnacles, Gooseneck_Barnacles, Anemone) %>%  #select only these columns
...
ggplot()+
  geom_col(data=clean_intertidal, aes(x=Tide_Height2, y=Crabs, fill=Site))+ #select the clean_intertidal dataframe and Tide_Height as my x axis, Number of Crabs as my y axis, and fill color representing Site
